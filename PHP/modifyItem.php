...
if (isset($_GET['item_ID'])) {
    ...
    if ($result_set) {
        if (mysqli_num_rows($result_set) == 1) {
            ...
            $fileName = $result['itemImage']; // Keep the original image in case no new image is uploaded
            $type = $result['type'];
        } else {
            //item not found
...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        ...
        if (!empty($_FILES['itemImgUpload']['name'])) { // Only update image if a new one was uploaded
        ...
        if ($result) {
            //query successful.... redirecting to admin dashboard
...
<script>
    function validateForm() {
        ...
        const genericnamePattern = /^[A-Za-z0-9\s\-\_\.\,]+$/; // Letters, numbers, and certain characters
        const brandnamePattern = /^[A-Za-z\s]+$/; // Only letters
        const pricePattern = /^[0-9]+$/; // Only numbers
